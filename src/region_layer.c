...
void delta_region_class(float *output, float *delta, int index, int classs, int classes, tree *hier, float scale, int stride, float *avg_cat)
{
    int i, n;
    if(hier){
        float pred = 1;
    ...
    for (b = 0; b < l.batch; ++b) {
        if(l.softmax_tree){
            ...
            for(t = 0; t < 30; ++t){
                ...
                if(!truth.x) break;
                int classs = state.truth[t*5 + b*l.truths + 4];
                float maxp = 0;
                int maxi = 0;
                if(truth.x > 100000 && truth.y > 100000){
                    ...
                    delta_region_class(l.output, l.delta, class_index, classs, l.classes, l.softmax_tree, l.class_scale, l.w*l.h, &avg_cat);
